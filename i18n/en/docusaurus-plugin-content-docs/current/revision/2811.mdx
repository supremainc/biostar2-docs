...
1. A 'TypeError: Cannot read property 'replace' of undefined' error occurred in the browser console when issuing a CSN card to users. <Badge>Affects version: v2.8.9</Badge>

1. Fingerprint authentication failed on the device where the data was transferred from BioStar 2 via Data File Export. <Badge>Affects version: v2.8.10</Badge>

1. When performing Import Event Logs from the device with a compatible version of firmware installed, a compatibility error occurred and it was not able to import data. <Badge>Affects version: v2.8.10</Badge>

1. It was not able to log in with users registered in the BioStar 2 version 2.6.4 when upgrading the version to 2.8.9 or later. <Badge>Affects version: v2.8.9</Badge>
