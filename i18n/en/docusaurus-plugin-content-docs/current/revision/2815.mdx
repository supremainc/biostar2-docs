...
1. DB Lock occurred when user data changes frequently, such as adding, deleting, or editing, were made in a short period of time during Automatic User Synchronization <Badge>2.7.14</Badge>.

1. In the MS SQL Server database environment, BioStar 2 did not operate normally if more than 526 user groups or users were set in the access group when using 'Specific Devices' Automatic User Synchronization <Badge>2.8.14</Badge>.

1. The data of the already deleted devices were logged in the user synchronization fail table when using 'Specific Devices' Automatic User Synchronization <Badge>2.7.11</Badge>.
