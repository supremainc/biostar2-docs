...
1. **REPORT** menu is not available when using a separate domain and private certificate. <Badge>2.9.3</Badge>

1. **REPORT** menu is not available when installed on an Azure database. <Badge>2.9.3</Badge>

1. When printing Unicode characters on cards using **Card Printer**, the characters are printed garbled. <Badge>2.9.3</Badge>

1. The operation fails if it takes more than 1 minute when using **Unified Gateway**. <Badge>2.9.3</Badge>

1. In an environment where multiple OM-120s are connected to the master device and the device is synchronized using the **Automatic User Synchronization** - **Specific Devices** mode and then authenticated, the relay malfunctions. <Badge>2.7.14</Badge>
